...
export const SWAGGER_ACCESS_RULES: Record<string, SwaggerAccessRule> = {
  ...
  'GET /api/v1/customer/home': { allowedRoles: 'REGISTERED_USER or GUEST_USER', description: 'Registered users receive personalized marketplace fields such as wishlist state, default address, and upcoming reminders where applicable. Guest users receive guest-safe marketplace data only. Guest users cannot access wishlist, cart, checkout, addresses, contacts, events, orders, payments, wallet, recurring payments, chats, reviews, or referrals. Guest response behavior includes isWishlisted=false, requiresAuthForWishlist=true, requiresAuthForCart=true, requiresAuthForCheckout=true, defaultAddress=null, upcomingReminder=null, and mode="GUEST" where applicable.' },
  'GET /api/v1/customer/categories': { allowedRoles: 'REGISTERED_USER or GUEST_USER', description: 'Registered users receive personalized marketplace fields such as wishlist state, default address, and upcoming reminders where applicable. Guest users receive guest-safe marketplace data only. Guest users cannot access wishlist, cart, checkout, addresses, contacts, events, orders, payments, wallet, recurring payments, chats, reviews, or referrals. Guest response behavior includes isWishlisted=false, requiresAuthForWishlist=true, requiresAuthForCart=true, requiresAuthForCheckout=true, defaultAddress=null, upcomingReminder=null, and mode="GUEST" where applicable.' },
  'GET /api/v1/customer/gifts': { allowedRoles: 'REGISTERED_USER or GUEST_USER', description: 'Registered users receive personalized marketplace fields such as wishlist state, default address, and upcoming reminders where applicable. Guest users receive guest-safe marketplace data only. Guest users cannot access wishlist, cart, checkout, addresses, contacts, events, orders, payments, wallet, recurring payments, chats, reviews, or referrals. Guest response behavior includes isWishlisted=false, requiresAuthForWishlist=true, requiresAuthForCart=true, requiresAuthForCheckout=true, defaultAddress=null, upcomingReminder=null, and mode="GUEST" where applicable.' },
  'GET /api/v1/customer/gifts/discounted': { allowedRoles: 'REGISTERED_USER or GUEST_USER', description: 'Registered users receive personalized marketplace fields such as wishlist state, default address, and upcoming reminders where applicable. Guest users receive guest-safe marketplace data only. Guest users cannot access wishlist, cart, checkout, addresses, contacts, events, orders, payments, wallet, recurring payments, chats, reviews, or referrals. Guest response behavior includes isWishlisted=false, requiresAuthForWishlist=true, requiresAuthForCart=true, requiresAuthForCheckout=true, defaultAddress=null, upcomingReminder=null, and mode="GUEST" where applicable.' },
  'GET /api/v1/customer/gifts/filter-options': { allowedRoles: 'REGISTERED_USER or GUEST_USER', description: 'Registered users receive personalized marketplace fields such as wishlist state, default address, and upcoming reminders where applicable. Guest users receive guest-safe marketplace data only. Guest users cannot access wishlist, cart, checkout, addresses, contacts, events, orders, payments, wallet, recurring payments, chats, reviews, or referrals. Guest response behavior includes isWishlisted=false, requiresAuthForWishlist=true, requiresAuthForCart=true, requiresAuthForCheckout=true, defaultAddress=null, upcomingReminder=null, and mode="GUEST" where applicable.' },
  'GET /api/v1/customer/gifts/{id}': { allowedRoles: 'REGISTERED_USER or GUEST_USER', description: 'Registered users receive personalized marketplace fields such as wishlist state, default address, and upcoming reminders where applicable. Guest users receive guest-safe marketplace data only. Guest users cannot access wishlist, cart, checkout, addresses, contacts, events, orders, payments, wallet, recurring payments, chats, reviews, or referrals. Guest response behavior includes isWishlisted=false, requiresAuthForWishlist=true, requiresAuthForCart=true, requiresAuthForCheckout=true, defaultAddress=null, upcomingReminder=null, and mode="GUEST" where applicable.' },
  ...
  'GET /api/v1/admin/reviews/moderation-logs': { allowedRoles: 'SUPER_ADMIN or ADMIN with reviewModerationLogs.read', description: 'SUPER_ADMIN or ADMIN with reviewModerationLogs.read permission.' },
  ...
  'GET /api/v1/admin/message-moderation/audit-logs': { allowedRoles: 'SUPER_ADMIN or ADMIN with messageModeration.auditLogs.read', description: 'SUPER_ADMIN or ADMIN with messageModeration.auditLogs.read permission.' },
  'POST /api/v1/admin/message-moderation/messages/{messageId}/block': { allowedRoles: 'SUPER_ADMIN or ADMIN with messageModeration.moderate', description: 'SUPER_ADMIN or ADMIN with messageModeration.moderate permission. Hide a flagged message from chat participants. Does not block the sender account.' },
  'POST /api/v1/admin/message-moderation/messages/{messageId}/restore': { allowedRoles: 'SUPER_ADMIN or ADMIN with messageModeration.moderate', description: 'SUPER_ADMIN or ADMIN with messageModeration.moderate permission. Restores a message hidden by moderation.' },
  'POST /api/v1/admin/message-moderation/messages/{messageId}/warn-user': { allowedRoles: 'SUPER_ADMIN or ADMIN with messageModeration.warn', description: 'SUPER_ADMIN or ADMIN with messageModeration.warn permission. Creates warning notification and audit log.' },
  'POST /api/v1/admin/message-moderation/messages/{messageId}/suspend-account': { allowedRoles: 'SUPER_ADMIN or ADMIN with messageModeration.suspend', description: 'SUPER_ADMIN or ADMIN with messageModeration.suspend permission. Refuses ADMIN/SUPER_ADMIN accounts.' },
  'POST /api/v1/admin/message-moderation/messages/{messageId}/dismiss-flag': { allowedRoles: 'SUPER_ADMIN or ADMIN with messageModeration.moderate', description: 'SUPER_ADMIN or ADMIN with messageModeration.moderate permission.' },
  'POST /api/v1/admin/message-moderation/messages/{messageId}/note': { allowedRoles: 'SUPER_ADMIN or ADMIN with messageModeration.notes.create', description: 'SUPER_ADMIN or ADMIN with messageModeration.notes.create permission. Internal-only note.' },
  'POST /api/v1/admin/message-moderation/messages/{messageId}/reprocess': { allowedRoles: 'SUPER_ADMIN or ADMIN with messageModeration.reprocess', description: 'SUPER_ADMIN or ADMIN with messageModeration.reprocess permission.' },
  'POST /api/v1/admin/message-moderation/messages/{messageId}/escalate': { allowedRoles: 'SUPER_ADMIN or ADMIN with messageModeration.escalate', description: 'SUPER_ADMIN or ADMIN with messageModeration.escalate permission.' },
  ...
  'POST /api/v1/admin/provider-payouts/{id}/approve': { allowedRoles: 'SUPER_ADMIN or ADMIN with providerPayouts.approve', description: 'SUPER_ADMIN or ADMIN with providerPayouts.approve permission. Idempotent for already approved payouts.' },
  'POST /api/v1/admin/provider-payouts/{id}/hold': { allowedRoles: 'SUPER_ADMIN or ADMIN with providerPayouts.hold', description: 'SUPER_ADMIN or ADMIN with providerPayouts.hold permission. Keeps ledger balance locked.' },
  'POST /api/v1/admin/provider-payouts/{id}/reject': { allowedRoles: 'SUPER_ADMIN or ADMIN with providerPayouts.reject', description: 'SUPER_ADMIN or ADMIN with providerPayouts.reject permission. Releases locked ledger balance.' },
  'POST /api/v1/admin/provider-payouts/bulk-approve': { allowedRoles: 'SUPER_ADMIN or ADMIN with providerPayouts.approve', description: 'SUPER_ADMIN or ADMIN with providerPayouts.approve permission. Returns per-item idempotent results.' },
  ...
  'GET /api/v1/admin/refund-policy-settings': { allowedRoles: 'SUPER_ADMIN or ADMIN with refundPolicies.read', description: 'SUPER_ADMIN or ADMIN with refundPolicies.read permission. Settings feed refund eligibility, auto-refund, dispute, and provider refund workflows.' },
  'PATCH /api/v1/admin/refund-policy-settings': { allowedRoles: 'SUPER_ADMIN', description: 'SUPER_ADMIN only. Updates global refund rules.' },
