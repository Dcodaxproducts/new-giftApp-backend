...
export class AuditLogWriterService {
  ...
  private module(action: string): string { if (action.startsWith('PROVIDER_')) return 'Provider Management'; if (action.startsWith('USER_') || action.startsWith('REGISTERED_USER_')) return 'User Management'; if (action.startsWith('ADMIN_') || action.startsWith('ROLE_')) return 'Admin Management'; if (action.startsWith('GIFT_')) return 'Gift Management'; if (action.startsWith('PAYMENT_') || action.startsWith('TRANSACTION_')) return 'Transaction Monitoring'; if (action.startsWith('DISPUTE_')) return 'Dispute Manager'; if (action.startsWith('PROVIDER_DISPUTE_')) return 'Provider Disputes'; if (action.startsWith('MEDIA_')) return 'Media Upload Policy'; if (action.startsWith('REFUND_POLICY_')) return 'Refund Policy Settings'; if (action.startsWith('REFERRAL_')) return 'Referral Settings'; if (action.startsWith('BROADCAST_')) return 'Broadcast Notifications'; if (action.startsWith('COUPON_')) return 'Coupons'; return 'System'; }
