...
export const SWAGGER_ACCESS_RULES: Record<string, SwaggerAccessRule> = {
  ...
  'GET /api/v1/chats': { allowedRoles: 'REGISTERED_USER or PROVIDER', description: 'Lists order-based conversations for the authenticated buyer or provider.' },
  'POST /api/v1/chats': { allowedRoles: 'REGISTERED_USER or PROVIDER', description: 'Start or get a conversation for an order. Only buyer and provider of the order can access.' },
  'GET /api/v1/chats/{id}': { allowedRoles: 'REGISTERED_USER or PROVIDER', description: 'Get conversation details. Only participants can access.' },
  'GET /api/v1/chats/{id}/messages': { allowedRoles: 'REGISTERED_USER or PROVIDER', description: 'Get paginated messages for a conversation.' },
  'POST /api/v1/chats/{id}/messages': { allowedRoles: 'REGISTERED_USER or PROVIDER', description: 'Send a message in a conversation.' },
  'PATCH /api/v1/chats/{id}/read': { allowedRoles: 'REGISTERED_USER or PROVIDER', description: 'Mark unread messages as read.' },
