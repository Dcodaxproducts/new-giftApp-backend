...
import { NotificationRecipientType, OrderStatus, PaymentMethod, PaymentStatus, Prisma, ProviderOrderRejectReason, ProviderOrderStatus, UserRole } from '@prisma/client';
...
export class ProviderOrdersService {
  ...
  private statusTitle(status: ProviderOrderStatus): string { return status.split('_').map((part) => part.charAt(0) + part.slice(1).toLowerCase()).join(' '); }
