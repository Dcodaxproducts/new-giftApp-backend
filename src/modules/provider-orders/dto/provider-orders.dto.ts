...
export class ProviderOrderActionDto { @ApiProperty({ enum: ProviderOrderAction, example: ProviderOrderAction.ACCEPT }) @IsEnum(ProviderOrderAction) action!: ProviderOrderAction; @ApiPropertyOptional({ enum: ProviderOrderStatus, example: ProviderOrderStatus.PROCESSING }) @IsOptional() @IsEnum(ProviderOrderStatus) status?: ProviderOrderStatus; @ApiPropertyOptional({ enum: ProviderOrderRejectReason, example: ProviderOrderRejectReason.OUT_OF_STOCK }) @IsOptional() @IsEnum(ProviderOrderRejectReason) reason?: ProviderOrderRejectReason; @ApiPropertyOptional({ example: 'Order accepted.' }) @IsOptional() @IsString() comment?: string; @ApiPropertyOptional({ example: 'Package handed to courier.' }) @IsOptional() @IsString() note?: string; @ApiPropertyOptional({ example: 'FedEx' }) @IsOptional() @IsString() carrier?: string; @ApiPropertyOptional({ example: 'FDX-123456' }) @IsOptional() @IsString() trackingNumber?: string; @ApiPropertyOptional({ example: '2026-10-26T10:00:00.000Z' }) @IsOptional() @IsISO8601() estimatedDeliveryAt?: string; @ApiPropertyOptional({ example: '2026-10-25T10:00:00.000Z' }) @IsOptional() @IsISO8601() dispatchAt?: string; @ApiPropertyOptional({ example: true }) @IsOptional() @IsBoolean() notifyCustomer?: boolean; }
