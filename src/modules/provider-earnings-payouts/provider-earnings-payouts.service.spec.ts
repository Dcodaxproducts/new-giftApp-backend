...
import { OrderStatus, PaymentStatus, ProviderEarningsLedgerDirection, ProviderEarningsLedgerStatus, ProviderEarningsLedgerType, ProviderPayoutAccountType, ProviderPayoutExternalProvider, ProviderPayoutMethodType, ProviderPayoutStatus, ProviderPayoutVerificationStatus, UserRole, UserStatus } from '@prisma/client';
...
const provider = { id: 'provider_1', role: UserRole.PROVIDER, deletedAt: null, status: UserStatus.APPROVED, isActive: true, isApproved: true, suspendedAt: null };
...
const providerOrder = { id: 'po_1', providerId: 'provider_1', orderId: 'order_1', orderNumber: 'PO-10293', totalPayout: 1200, total: 1500, currency: 'USD', status: OrderStatus.DELIVERED, order: { orderNumber: 'ORD-10293', paymentStatus: PaymentStatus.SUCCEEDED } };
...
function createService(overrides: Partial<{ ledgers: unknown[]; payoutMethod: unknown; payout: unknown; duplicate: unknown; providerOrder: unknown; provider: unknown }> = {}) {
  ...
  const prisma = {
    ...
    providerPayoutAuditLog: { create: jest.fn() },
...
describe('Provider earnings/payouts source safety', () => {
  ...
  it('adds repository write methods while service keeps payout decisions', () => {
    ...
    expect(repository).toContain('markLedgerEntriesPayoutPending');
  ...
  it('service no longer contains direct Prisma writes for payout lifecycle methods', () => {
    expect(service).not.toContain('providerEarningsLedger.upsert');
...
describe('ProviderEarningsPayoutsService', () => {
  ...
  it('payout request locks ledger balance', async () => { const { service, prisma } = createService(); await service.requestPayout({ uid: 'provider_1', role: UserRole.PROVIDER }, { amount: 500, payoutMethodId: 'method_1', idempotencyKey: 'idem_new' }); expect(prisma.providerEarningsLedger.updateMany).toHaveBeenCalledWith(expect.objectContaining({ where: expect.objectContaining({ id: { in: ['ledger_1'] }, status: ProviderEarningsLedgerStatus.AVAILABLE }), data: expect.objectContaining({ payoutId: 'payout_1', status: ProviderEarningsLedgerStatus.PAYOUT_PENDING }) })); });
  it('recordOrderEarning creates expected ledger entry', async () => { const { service, prisma } = createService(); await service.recordOrderEarning('po_1'); expect(prisma.providerEarningsLedger.upsert).toHaveBeenCalledWith(expect.objectContaining({ where: { providerOrderId_type: { providerOrderId: 'po_1', type: ProviderEarningsLedgerType.ORDER_EARNING } }, update: {}, create: expect.objectContaining({ providerId: 'provider_1', providerOrderId: 'po_1', type: ProviderEarningsLedgerType.ORDER_EARNING, direction: ProviderEarningsLedgerDirection.CREDIT, amount: 1200, currency: 'USD', status: ProviderEarningsLedgerStatus.AVAILABLE, metadataJson: { orderId: 'order_1' } }) })); });
  it('recordOrderEarning does not duplicate existing earning because repository uses upsert', async () => { const { service, prisma } = createService(); await service.recordOrderEarning('po_1'); expect(prisma.providerEarningsLedger.upsert).toHaveBeenCalledWith(expect.objectContaining({ update: {} })); });
  it('recordOrderEarning skips unpaid or incomplete orders', async () => { const { service, prisma } = createService({ providerOrder: { ...providerOrder, status: OrderStatus.ACCEPTED } }); await service.recordOrderEarning('po_1'); expect(prisma.providerEarningsLedger.upsert).not.toHaveBeenCalled(); });
