...
import { CancellationTierDto, UpdateRefundPolicySettingsDto } from './dto/refund-policy-settings.dto';

type SettingsWithUpdater = RefundPolicySettings & { updatedBy?: { id: string; firstName: string; lastName: string } | null };
type CancellationTier = { daysBeforeDelivery: number; deductionPercent: number; label: string };
...
export class RefundPolicySettingsService {
  ...
  private normalizeCancellationTiers(tiers: CancellationTierDto[]): StoredCancellationTier[] {
    ...
    return tiers.map((tier, index) => {
      ...
      if (seenDays.has(tier.daysBeforeDelivery)) throw new BadRequestException('Duplicate cancellation tier daysBeforeDelivery values are not allowed');
      seenDays.add(tier.daysBeforeDelivery);
      return { daysBeforeDelivery: tier.daysBeforeDelivery, deductionPercent: tier.deductionPercent, label, createdAt: new Date(Date.now() + index).toISOString() };
    });
  }
  ...
  private cancellationTiers(value: Prisma.JsonValue): CancellationTier[] {
    return this.storedCancellationTiers(value).map((tier) => ({ daysBeforeDelivery: tier.daysBeforeDelivery, deductionPercent: tier.deductionPercent, label: tier.label }));
  }
  private storedCancellationTiers(value: Prisma.JsonValue, fallbackCreatedAt = new Date()): StoredCancellationTier[] {
    ...
    return value
      ...
      .map((item, index) => ({
        daysBeforeDelivery: typeof item.daysBeforeDelivery === 'number' ? item.daysBeforeDelivery : typeof item.daysBeforeCheckIn === 'number' ? item.daysBeforeCheckIn : 0,
