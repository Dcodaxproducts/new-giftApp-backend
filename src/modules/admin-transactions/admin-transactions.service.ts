...
export class AdminTransactionsService {
  ...
  async refund(user: AuthUserContext, id: string, dto: RefundAdminTransactionDto) {
    ...
    const policy = await this.refundPolicy.getActivePolicy();
    if (!policy.allowCancellation) throw new BadRequestException('Cancellation/refund is currently disabled');
  ...
  private paymentWhere(query: Partial<ListAdminTransactionsDto>): Prisma.PaymentWhereInput {
    ...
    if (query.providerId) where.order = { providerId: query.providerId };
    return where;
  }

  private paymentInclude() { return { user: { select: { id: true, firstName: true, lastName: true, email: true, avatarUrl: true } }, order: { include: { provider: { select: { id: true, providerProfile: { select: { businessName: true } } } } } }, moneyGift: true, customerSubscription: true, recurringPaymentOccurrences: { include: { recurringPayment: true } }, refundRequests: true } satisfies Prisma.PaymentInclude; }
  ...
  private normalize(payment: PaymentRecord): NormalizedAdminTransaction {
    ...
    const providerOrder = payment.order;
    const type = this.type(payment, metadata);
    return { id: payment.id, transactionId: this.transactionId(payment), paymentId: payment.id, orderId: payment.orderId, orderNumber: payment.order?.orderNumber ?? null, userId: payment.userId, user: { id: payment.user.id, name: this.name(payment.user), email: payment.user.email, avatarUrl: payment.user.avatarUrl }, providerId: providerOrder?.providerId ?? null, providerBusinessName: providerOrder?.provider.providerProfile?.businessName ?? null, gatewayProvider: this.gateway(payment), type, amount: this.money(payment.amount), currency: payment.currency, status, paymentStatus: payment.status, paymentMethod: payment.paymentMethod, gatewayReference: payment.providerPaymentIntentId, metadata, subtotal: payment.order ? this.money(payment.order.subtotal) : this.money(payment.amount), discount: 0, deliveryFee: 0, tax: 0, createdAt: payment.createdAt };
  }
  ...
  private matches(item: NormalizedAdminTransaction, query: Partial<ListAdminTransactionsDto>): boolean {
    ...
    const haystack = [item.user.name, item.user.email].filter(Boolean).join(' ').toLowerCase();
