...
export class AdminTransactionsService {
  ...
  async refund(user: AuthUserContext, id: string, dto: RefundAdminTransactionDto) {
    ...
    const policyResult = await this.refundPolicy.evaluateRefundEligibility({ deliveredAt: this.deliveredAt(providerOrder.updatedAt, item.createdAt), requestedAmount: refundAmount, remainingRefundableAmount, paymentRefundable: true });
  ...
  private deliveredAt(date: Date | null, fallback: Date): Date { return date ?? fallback; }
