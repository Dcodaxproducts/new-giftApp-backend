...
import { AccountType, MessageModerationAction, MessageModerationCase, MessageModerationEscalation, MessageModerationFlagType, MessageModerationSeverity, MessageModerationSource, MessageModerationStatus, NotificationRecipientType, Prisma, UserRole } from '@prisma/client';
...
type CaseWithLogs = MessageModerationCase & { escalations?: MessageModerationEscalation[] };
type Sender = { id: string; role: UserRole; firstName: string; lastName: string; email: string; avatarUrl: string | null; providerProfile?: { businessName: string | null } | null };
...
export class MessageModerationService {
  ...
  private async suspend(user: AuthUserContext, messageId: string, dto: MessageModerationActionDto) {
    ...
    if (sender.role === UserRole.STAFF || sender.role === UserRole.SUPER_ADMIN) throw new ForbiddenException('Admin and Super Admin accounts cannot be suspended from message moderation.');
  ...
  private auditWhere(dto: ListMessageModerationAuditLogsDto): Record<string, unknown> { return {}; }
  ...
  private recipientType(role: string): NotificationRecipientType { return role === UserRole.PROVIDER ? NotificationRecipientType.PROVIDER : role === UserRole.STAFF || role === UserRole.SUPER_ADMIN ? NotificationRecipientType.ADMIN : NotificationRecipientType.REGISTERED_USER; }
