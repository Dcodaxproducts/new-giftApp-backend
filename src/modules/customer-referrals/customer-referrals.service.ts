...
import { Prisma, Referral, ReferralStatus, RewardLedgerSource, RewardLedgerType, User, UserRole } from '@prisma/client';
...
export class CustomerReferralsService {
  constructor(
    private readonly referralsRepository: CustomerReferralsRepository,
    private readonly rewardsRepository: CustomerRewardsRepository,
  ...
  async summary(user: AuthUserContext) {
    const [referrals, ledger] = await Promise.all([
      this.referralsRepository.findReferralSummaryForUser(user.uid),
      this.rewardsRepository.findRewardLedgerForUser(user.uid),
    ]);
  ...
  async redeem(user: AuthUserContext, dto: RedeemRewardDto) {
    ...
    const entry = await this.rewardsRepository.createRewardRedemption({ userId: user.uid, type: RewardLedgerType.REDEEMED, amount: new Prisma.Decimal(amount), currency: balance.currency, source: RewardLedgerSource.REFERRAL, sourceId: `redeem:${user.uid}:${Date.now()}:${randomInt(1000, 9999)}`, description: `Reward redeemed to ${dto.redeemTo}.` });
  ...
  async assertValidReferralCode(referralCode?: string): Promise<void> {
    ...
    if (!code) return;
    const referrer = await this.referralsRepository.findReferrerByCode(code);
  ...
  async awardReferralForFirstEligiblePurchase(referredUserId: string, sourceId: string, transactionAmount = 0): Promise<void> {
    const referral = await this.referralsRepository.findReferralForReward(referredUserId);
    if (!referral || referral.status === ReferralStatus.REWARDED || referral.status === ReferralStatus.EXPIRED) return;
    const settings = await this.referralSettingsService.getActiveSettings();
    if (!settings.isActive) return;
    if (referral.expiresAt && referral.expiresAt < new Date()) { await this.referralsRepository.updateReferralStatus(referral.id, { status: ReferralStatus.EXPIRED }); return; }
    if (transactionAmount < Number(referral.minimumTransactionAmountSnapshot)) return;
    const existingReward = await this.rewardsRepository.findExistingReferralReward(referral.referrerUserId, referral.id);
    ...
    await this.rewardsRepository.createReferralRewardEntries({ where: { id: referral.id }, data: { status: ReferralStatus.REWARDED, qualifiedAt: referral.qualifiedAt ?? new Date(), rewardedAt: new Date(), rewardAmount: new Prisma.Decimal(amount), currency } }, ledgers);
  ...
  private async rewardBalance(userId: string) {
    const items = await this.rewardsRepository.sumRewardBalanceForUser(userId);
  ...
  private async getOrCreateReferralCode(userId: string): Promise<string> {
    const user = await this.referralsRepository.findOrCreateReferralCode(userId);
    ...
    for (let attempt = 0; attempt < 5; attempt += 1) {
      ...
      try {
        const updated = await this.referralsRepository.setReferralCode(userId, code);
  ...
  private async notify(recipientId: string, title: string, message: string, type: string, metadata: Prisma.InputJsonObject): Promise<void> { await this.referralsRepository.createNotification(recipientId, title, message, type, metadata); }
}
