...
describe('Promotional offers repository cleanup', () => {
  const service = readFileSync(join(__dirname, 'promotional-offers.service.ts'), 'utf8');
  const adminRepository = readFileSync(join(__dirname, 'promotional-offers.repository.ts'), 'utf8');
  const providerRepository = readFileSync(join(__dirname, 'provider-offers.repository.ts'), 'utf8');
  const moduleFile = readFileSync(join(__dirname, 'promotional-offers.module.ts'), 'utf8');
  const adminController = readFileSync(join(__dirname, 'promotional-offers-management.controller.ts'), 'utf8');
  const providerController = readFileSync(join(__dirname, 'provider-promotional-offers.controller.ts'), 'utf8');
