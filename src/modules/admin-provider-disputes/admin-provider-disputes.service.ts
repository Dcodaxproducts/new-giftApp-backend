...
export class AdminProviderDisputesService {
  ...
  async stats(query: ProviderDisputeDateRangeDto) {
    ...
    const [criticalOpenCases, evidencePhase, underReview, escalations, resolvedThisWeek, closureRows, topProvider, totalProviderDisputes] = await this.disputesRepository.countStats({ where, weekStart });
    const topConflict = topProvider[0];
    const provider = topConflict ? await this.disputesRepository.findProviderBusinessName(topConflict.providerId) : null;
    ...
    return { data: { criticalOpenCases, evidencePhase, underReview, escalations, resolvedThisWeek, averageClosureTimeDays, topConflictSource: topConflict ? { providerName: provider?.providerBusinessName ?? 'Unknown Provider', category: topConflict.category, percentOfTotal: this.percent(this.groupCount(topConflict._count), totalProviderDisputes) } : null, systemHealth: null }, message: 'Provider dispute stats fetched successfully.' };
  }
  ...
  async evidence(id: string) {
    ...
    return { data: { caseId: dispute.caseId, reviewStatus: { startedBy: dispute.assignedTo ? this.initials(dispute.assignedTo) : null, startedAt: dispute.evidenceReviewStartedAt ?? null, isComplete: Boolean(dispute.evidenceReviewCompletedAt) }, customerEvidence: customerEvidence ? this.evidenceBlock(customerEvidence) : null, providerEvidence: providerEvidence ? this.evidenceBlock(providerEvidence) : null, internalReviewerNotes: null }, message: 'Provider dispute evidence fetched successfully.' };
  }
  ...
  private performanceImpact(finalRuling: ProviderDisputeFinalRuling) { return { winRateBefore: null, winRateAfter: null, penaltyPoints: finalRuling === ProviderDisputeFinalRuling.PROVIDER_WINS ? 0 : 1, tierStatus: null }; }
  ...
  private computeFinancialImpact(dispute: ProviderDisputeView) {
    ...
    return { caseId: dispute.caseId, ruling: dispute.ruling, providerAccountPreview: { currentBalance, pendingPayout, newBalance: null, currency: dispute.currency }, breakdown, totalProviderDeduction };
  }
  ...
  private percent(count: number, total: number): number { return total === 0 ? 0 : Math.round((count / total) * 100); }
  private groupCount(count: true | { _all?: number } | undefined): number { return !count || count === true ? 0 : count._all ?? 0; }
  private evidenceSummary(evidence: { submittedByType: string; narrative: string | null }[]) {
    const customer = evidence.find((item) => item.submittedByType === 'CUSTOMER');
    const provider = evidence.find((item) => item.submittedByType === 'PROVIDER');
    return { customer: customer?.narrative ?? null, provider: provider?.narrative ?? null };
  }
  ...
  private listItem(item: ProviderDisputeView) { return { id: item.id, caseId: item.caseId, provider: { id: item.provider.id, businessName: item.provider.providerBusinessName ?? this.name(item.provider), contactName: this.name(item.provider), tier: null }, customer: { id: item.customer.id, name: this.name(item.customer) }, transaction: { id: item.payment?.id ?? item.transactionId, transactionId: item.transactionId ?? item.payment?.providerPaymentIntentId, status: item.payment?.status ?? null }, category: item.category, amount: this.money(item.amount), currency: item.currency, status: item.status, priority: item.priority, riskAssessment: item.riskScore >= 60 ? 'HIGH' : item.riskScore >= 30 ? 'MEDIUM' : 'LOW', daysOpen: this.daysOpen(item), createdAt: item.createdAt }; }
  ...
  private files(value: Prisma.JsonValue) { return Array.isArray(value) ? value.map((item) => { const file = item && typeof item === 'object' && !Array.isArray(item) ? item as Record<string, unknown> : {}; const contentType = typeof file.contentType === 'string' ? file.contentType : null; return { id: typeof file.id === 'string' ? file.id : null, fileName: typeof file.fileName === 'string' ? file.fileName : null, fileUrl: typeof file.fileUrl === 'string' ? file.fileUrl : null, contentType, sizeText: typeof file.sizeText === 'string' ? file.sizeText : null, category: contentType ? this.fileCategory(contentType) : null }; }) : []; }
