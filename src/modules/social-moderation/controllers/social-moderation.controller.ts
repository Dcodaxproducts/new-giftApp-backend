...
import { CreateSocialReportingRuleDto, ExportSocialReportsDto, ExportSocialRulesDto, ListSocialReportingRulesDto, ListSocialReportsDto, SocialModerationActionDto, SocialModerationStatsDto, UpdateSocialReportingRuleDto } from '../dto/social-moderation.dto';
...
export class SocialReportingRulesController extends SocialModerationBaseController {
  ...
  @Patch(':id') @Permissions('socialReportingRules.update') @ApiOperation({ summary: 'Update social reporting rule', description: 'SUPER_ADMIN or ADMIN with socialReportingRules.update. Standard rule fields and isActive status changes both go through this endpoint.' }) @ApiBody({ type: UpdateSocialReportingRuleDto, examples: { updateRule: { value: { label: 'Spam', description: 'Spam or advertising content.', iconKey: 'spam', autoFlagThreshold: 10, escalationRule: 'MANUAL_REVIEW' } }, activateRule: { value: { label: 'Spam', isActive: true, reason: 'Rule enabled for campaign period.' } }, deactivateRule: { value: { label: 'Spam', isActive: false, reason: 'Rule paused after campaign period.' } } } }) @ApiResponse({ status: 200, description: 'Social reporting rule updated successfully', schema: { example: { success: true, data: { id: 'rule_1', label: 'Spam', isActive: true, autoFlagThreshold: 10, escalationRule: 'MANUAL_REVIEW' }, message: 'Social reporting rule updated successfully.' } } }) update(@CurrentUser() user: AuthUserContext, @Param('id') id: string, @Body() dto: UpdateSocialReportingRuleDto) { return this.service.updateRule(user, id, dto); }
  @Delete(':id') @Permissions('socialReportingRules.delete') @ApiOperation({ summary: 'Delete social reporting rule', description: 'SUPER_ADMIN or ADMIN with socialReportingRules.delete. Permanently deletes the rule record.' }) remove(@CurrentUser() user: AuthUserContext, @Param('id') id: string) { return this.service.deleteRule(user, id); }
}
