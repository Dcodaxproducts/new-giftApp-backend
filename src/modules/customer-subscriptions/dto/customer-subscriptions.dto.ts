...
import { IsBoolean, IsEnum, IsInt, IsOptional, IsString, Min, Max, MaxLength } from 'class-validator';
...
export class SubscriptionCheckoutDto { @ApiProperty() @IsString() planId!: string; @ApiProperty({ enum: BillingCycle }) @IsEnum(BillingCycle) billingCycle!: BillingCycle; @ApiProperty({ enum: PaymentMethod, example: PaymentMethod.STRIPE_CARD }) @IsEnum(PaymentMethod) paymentMethod!: PaymentMethod; @ApiPropertyOptional({ example: 'pm_xxx' }) @IsOptional() @IsString() stripePaymentMethodId?: string; @ApiPropertyOptional({ example: 'sub_checkout_2026_001' }) @IsOptional() @IsString() @MaxLength(120) idempotencyKey?: string; }
