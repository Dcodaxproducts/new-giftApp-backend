...
describe('Subscription plans repository cleanup', () => {
  const service = readFileSync(join(__dirname, '../services/subscription-plans.service.ts'), 'utf8');
  const plansRepository = readFileSync(join(__dirname, '../repositories/subscription-plans.repository.ts'), 'utf8');
  const featuresRepository = readFileSync(join(__dirname, '../repositories/plan-features.repository.ts'), 'utf8');
  const couponsRepository = readFileSync(join(__dirname, '../repositories/coupons.repository.ts'), 'utf8');
  const moduleFile = readFileSync(join(__dirname, '../subscription-plans.module.ts'), 'utf8');
  const plansController = readFileSync(join(__dirname, '../controllers/subscription-plans.controller.ts'), 'utf8');
  const featuresController = readFileSync(join(__dirname, '../controllers/plan-features.controller.ts'), 'utf8');
  const couponsController = readFileSync(join(__dirname, '../controllers/coupons.controller.ts'), 'utf8');
  ...
  it('moves plan, feature, and coupon persistence into repositories', () => {
    ...
    ['findCouponsAndCount', 'findCouponById', 'findCouponByCode', 'createCoupon', 'updateCoupon', 'deleteCoupon'].forEach((method) => expect(couponsRepository).toContain(method));
  });
