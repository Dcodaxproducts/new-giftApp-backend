...
export class CouponsController {
  ...
  @ApiOperation({
    ...
    description: 'By default returns all non-deleted coupons. Use status to filter ACTIVE, INACTIVE, or EXPIRED coupons.',
  })
  ...
  create(@CurrentUser() user: AuthUserContext, @Body() dto: CreateCouponDto) {
    return this.service.createCoupon(user, dto);
  }

  @Get(':id')
  @Permissions('coupons.read')
  details(@Param('id') id: string) {
    return this.service.couponDetails(id);
  }
  ...
  @ApiOperation({
    ...
    description:
      "SUPER_ADMIN or ADMIN with coupon-specific permissions. Standard coupon fields require 'coupons.update'; status or isActive changes require 'coupons.status.update'; mixed payloads require both permissions.",
  })
  @ApiBody({
  ...
  update(@CurrentUser() user: AuthUserContext, @Param('id') id: string, @Body() dto: UpdateCouponDto) {
    return this.service.updateCoupon(user, id, dto);
  }

  @Delete(':id')
  @Permissions('coupons.delete')
  delete(@CurrentUser() user: AuthUserContext, @Param('id') id: string) {
    return this.service.deleteCoupon(user, id);
  }
}