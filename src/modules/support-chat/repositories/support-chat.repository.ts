...
export class SupportChatRepository { constructor(private readonly prisma: PrismaService) {}
  ...
  createAdminMessage(params: { supportChatId: string; senderId: string; clientMessageId?: string; messageType: string; body?: string | null; attachmentUrls: string[] }) { return this.prisma.$transaction(async (tx) => { if (params.clientMessageId) { const existing = await tx.supportChatMessage.findFirst({ where: { supportChatId: params.supportChatId, senderId: params.senderId, clientMessageId: params.clientMessageId } }); if (existing) return existing; } const message = await tx.supportChatMessage.create({ data: { supportChatId: params.supportChatId, senderId: params.senderId, senderType: SupportChatSenderType.ADMIN, clientMessageId: params.clientMessageId, messageType: params.messageType as never, body: params.body, attachmentUrlsJson: params.attachmentUrls, readByAdminAt: new Date() } }); await tx.supportChat.update({ where: { id: params.supportChatId }, data: { status: SupportChatStatus.ACTIVE, lastMessageId: message.id, lastMessageAt: message.createdAt, participantUnreadCount: { increment: 1 } } }); return message; }); }
