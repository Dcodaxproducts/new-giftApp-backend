...
import { CommissionTier, Prisma } from '@prisma/client';
...
export class AdminPayoutSettingsService {
  ...
  async get() {
    ...
    return { data: { commissionTiers: tiers.map((tier) => this.toTierView(tier)) }, message: 'Commission tiers fetched successfully.' };
  }
  ...
  private async writeAudit(user: AuthUserContext, targetId: string, action: string, targetType: string, beforeJson: unknown, afterJson: unknown, ipAddress?: string, _userAgent?: string | string[]) { await this.auditLog.write({ actorId: user.uid, targetId, targetType, action, module: 'Commission & Payout Settings', beforeJson, afterJson: { ...(afterJson && typeof afterJson === 'object' && !Array.isArray(afterJson) ? afterJson : { value: afterJson }), appliesToFuturePayoutsOnly: true, tierChangesEffectiveFrom: 'NEXT_BILLING_OR_PAYOUT_CYCLE' }, ipAddress }); }
