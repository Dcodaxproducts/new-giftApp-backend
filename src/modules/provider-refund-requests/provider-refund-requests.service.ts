...
import { Prisma, RefundRejectReason, RefundRequestStatus } from '@prisma/client';
...
export class ProviderRefundRequestsService {
  ...
  private shouldProcessAsync(_refund: RefundRequestView): boolean { return false; }
  private stripeRefundPlaceholder(_refund: RefundRequestView): string | null { return null; }
  ...
  private toDetails(item: RefundRequestView) { return { id: item.id, orderId: item.orderId, orderNumber: item.order.orderNumber, status: item.status, customerReason: item.customerReason, requestedAmount: Number(item.requestedAmount), currency: item.currency, requestedAt: item.requestedAt, evidence: this.evidence(item.evidenceUrlsJson), customer: { id: item.userId, name: this.name(item.user), email: item.user.email }, orderInfo: { orderNumber: item.order.orderNumber, orderDate: item.order.createdAt, deliveryStatus: item.order.status, subtotal: Number(item.order.subtotal), totalPaid: Number(item.order.total) }, items: item.order.items.map((orderItem) => ({ id: orderItem.id, name: orderItem.gift.name, quantity: orderItem.quantity, total: Number(orderItem.unitPrice) * orderItem.quantity, imageUrl: this.firstImage(orderItem.gift.imageUrls) })) }; }
