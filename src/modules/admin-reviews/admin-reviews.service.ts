...
export class AdminReviewsService {
  ...
  async details(id: string) {
    ...
    const lastLog = await this.prisma.reviewModerationLog.findFirst({ where: { reviewId: id }, orderBy: { createdAt: 'desc' } });
    return { data: { ...this.reviewListItem(review), customer: { id: review.customer.id, name: this.name(review.customer), email: review.customer.email }, order: { id: review.order.id, orderNumber: review.order.orderNumber, createdAt: review.order.createdAt }, moderation: { autoModerated: review.autoModerated, confidence: review.moderationConfidence ?? 0, detectedCategories: this.stringArray(review.detectedCategoriesJson), lastAction: lastLog?.action ?? null } }, message: 'Review details fetched successfully.' };
  }
  ...
  async export(query: ExportReviewsDto) {
    const where: Prisma.ReviewWhereInput = { deletedAt: null, status: query.status, severity: query.severity, providerId: query.providerId, rating: query.rating, ...(query.fromDate || query.toDate ? { createdAt: { ...(query.fromDate ? { gte: new Date(query.fromDate) } : {}), ...(query.toDate ? { lte: new Date(query.toDate) } : {}) } } : {}) };
    const items = await this.prisma.review.findMany({ where, include: this.reviewInclude(), orderBy: { createdAt: 'desc' }, take: 10000 });
    const rows = [['Review Code', 'Rating', 'Status', 'Severity', 'Flag Reason', 'Provider', 'Order Number', 'Has Provider Response', 'Created At'], ...items.map((item) => [item.reviewCode, String(item.rating), item.status, item.severity, item.flagReason ?? '', this.providerName(item.provider), item.order.orderNumber, item.response ? 'YES' : 'NO', item.createdAt.toISOString()])];
  ...
  private reviewWhere(query: ListReviewsDto): Prisma.ReviewWhereInput {
    return { deletedAt: null, rating: query.rating, providerId: query.providerId, userId: query.userId, orderId: query.orderId, ...(query.status && query.status !== AllReviewStatus.ALL ? { status: query.status } : {}), ...(query.severity && query.severity !== AllReviewSeverity.ALL ? { severity: query.severity } : {}), ...(query.hasProviderResponse !== undefined ? { response: query.hasProviderResponse ? { isNot: null } : { is: null } } : {}), ...(query.fromDate || query.toDate ? { createdAt: { ...(query.fromDate ? { gte: new Date(query.fromDate) } : {}), ...(query.toDate ? { lte: new Date(query.toDate) } : {}) } } : {}), ...(query.search ? { OR: [{ reviewCode: { contains: query.search, mode: 'insensitive' } }, { comment: { contains: query.search, mode: 'insensitive' } }, { provider: { providerBusinessName: { contains: query.search, mode: 'insensitive' } } }, { order: { orderNumber: { contains: query.search, mode: 'insensitive' } } }] } : {}) };
  }
  ...
  private reviewOrder(sortBy?: ReviewSortBy, sortOrder?: SortOrder): Prisma.ReviewOrderByWithRelationInput { const order = sortOrder === SortOrder.ASC ? 'asc' : 'desc'; if (sortBy === ReviewSortBy.RATING) return { rating: order }; if (sortBy === ReviewSortBy.SEVERITY) return { severity: order }; return { createdAt: order }; }
  private reviewListItem(review: ReviewWithRelations) { return { id: review.id, reviewCode: review.reviewCode, rating: review.rating, comment: review.comment, status: review.status, severity: review.severity, flagReason: review.flagReason, customer: { id: review.customer.id, name: this.name(review.customer), avatarUrl: review.customer.avatarUrl }, provider: { id: review.provider.id, businessName: this.providerName(review.provider) }, order: { id: review.order.id, orderNumber: review.order.orderNumber }, providerResponse: review.response ? { id: review.response.id, body: review.response.body, createdAt: review.response.createdAt } : null, createdAt: review.createdAt }; }
  private actionUpdate(dto: ModerateReviewDto): Prisma.ReviewUpdateInput { const map: Record<ReviewModerationAction, ReviewStatus> = { APPROVE: ReviewStatus.PUBLISHED, HIDE: ReviewStatus.HIDDEN, REMOVE: ReviewStatus.REMOVED, PENALIZE: ReviewStatus.PENALIZED, RESTORE: ReviewStatus.PUBLISHED, MARK_SPAM: ReviewStatus.HIDDEN, MARK_FAKE: ReviewStatus.REMOVED, AUTO_HIDDEN: ReviewStatus.HIDDEN, AUTO_APPROVED: ReviewStatus.PUBLISHED, AUTO_FLAGGED: ReviewStatus.FLAGGED }; if (!this.manualActions().has(dto.action)) throw new BadRequestException('Unsupported manual moderation action'); return { status: map[dto.action], flagReason: this.actionReason(dto) }; }
  ...
  private reviewSnapshot(review: Review) { return { id: review.id, status: review.status, severity: review.severity, flagReason: review.flagReason, moderationConfidence: review.moderationConfidence }; }
