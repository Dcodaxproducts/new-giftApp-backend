...
import { DispatchNotificationInput } from '../notifications/notification-dispatch.service';
...
type ReviewWithRelations = Review & {
  ...
  order: { id: string; orderNumber: string; createdAt: Date; payments: { id: string; providerPaymentIntentId: string | null; createdAt: Date }[] };
...
export class AdminReviewsService {
  ...
  private transactionId(review: ReviewWithRelations): string { const payment = review.order.payments[0]; return payment?.providerPaymentIntentId ?? `TXN-${payment?.createdAt.getUTCFullYear() ?? review.createdAt.getUTCFullYear()}-${(payment?.id ?? review.order.id).slice(-8).toUpperCase()}`; }
