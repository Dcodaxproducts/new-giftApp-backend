...
function createService(overrides: Partial<{ method: PayoutMethodFixture | null; pendingPayout: unknown; methods: PayoutMethodFixture[] }> = {}) {
  ...
  const prisma = {
    ...
    providerPayoutMethod: {
      ...
      findFirst: jest.fn().mockImplementation((args: { where?: { fingerprint?: string } }) => args.where?.fingerprint ? Promise.resolve(null) : Promise.resolve(method)),
...
describe('ProviderPayoutMethodsService', () => {
  ...
  it('delete hard-deletes own method and promotes next verified default only when needed', async () => {
    ...
    expect(prisma.providerPayoutMethod.update).toHaveBeenCalledWith(expect.objectContaining({ where: { id: 'payout_method_1' }, data: expect.objectContaining({ deletedAt: expect.any(Date), isActive: false, isDefault: false }) }));
