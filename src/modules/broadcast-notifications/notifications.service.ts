...
export class NotificationsService {
  constructor(private readonly prisma: PrismaService, private readonly auditLog: AuditLogWriterService, private readonly gateway: NotificationsGateway) {}
  ...
  async list(user: AuthUserContext, query: ListNotificationsDto) {
    ...
    const [items, total] = await this.prisma.$transaction([
      this.prisma.notification.findMany({ where, orderBy: { createdAt: query.sortOrder === SortOrder.ASC ? 'asc' : 'desc' }, skip: (page - 1) * limit, take: limit }),
  ...
  async summary(user: AuthUserContext) {
    ...
    const [total, unread, birthdays, deliveries, newContacts] = await this.prisma.$transaction([
      ...
      this.prisma.notification.count({ where: { ...base, type: NotificationTypeDto.BIRTHDAY_REMINDER } }),
      this.prisma.notification.count({ where: { ...base, type: NotificationTypeDto.GIFT_DELIVERED } }),
      this.prisma.notification.count({ where: { ...base, type: NotificationTypeDto.NEW_CONTACT_AVAILABLE } }),
    ]);
  ...
  async markAllRead(user: AuthUserContext) {
    await this.prisma.notification.updateMany({ where: { recipientId: user.uid, isRead: false, deletedAt: null }, data: { isRead: true, readAt: new Date() } });
  ...
  async updatePreferences(user: AuthUserContext, dto: UpdateNotificationPreferencesDto) {
    ...
    const updated = await this.prisma.notificationPreference.update({ where: { userId: user.uid }, data: { pushEnabled: dto.pushEnabled, emailEnabled: dto.emailEnabled, smsEnabled: dto.smsEnabled, dealUpdatesEnabled: dto.dealUpdatesEnabled, birthdayRemindersEnabled: dto.birthdayRemindersEnabled, deliveryUpdatesEnabled: dto.deliveryUpdatesEnabled, newContactAlertsEnabled: dto.newContactAlertsEnabled, providerOrderAlertsJson: dto.providerOrderAlerts, providerAccountActivityJson: dto.providerAccountActivity, providerMarketingUpdatesJson: dto.providerMarketingUpdates } });
  ...
  async saveDeviceToken(user: AuthUserContext, dto: DeviceTokenDto) {
    const token = await this.prisma.notificationDeviceToken.upsert({ where: { userId_deviceId: { userId: user.uid, deviceId: dto.deviceId } }, create: { userId: user.uid, token: dto.token, platform: dto.platform, deviceId: dto.deviceId, isActive: true, lastUsedAt: new Date() }, update: { token: dto.token, platform: dto.platform, isActive: true, lastUsedAt: new Date() } });
  ...
  private async getOwnedNotification(userId: string, id: string): Promise<Notification> { const notification = await this.prisma.notification.findFirst({ where: { id, recipientId: userId, deletedAt: null } }); if (!notification) throw new NotFoundException('Notification not found'); return notification; }
  private async getOrCreatePreferences(userId: string): Promise<NotificationPreference> { return (await this.prisma.notificationPreference.findUnique({ where: { userId } })) ?? this.prisma.notificationPreference.create({ data: { userId } }); }
  ...
  private async snooze(notification: Notification, metadata: NotificationMetadata): Promise<void> { const snoozedUntil = new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString(); await this.prisma.notification.update({ where: { id: notification.id }, data: { metadataJson: { ...metadata, snoozedUntil } } }); }
