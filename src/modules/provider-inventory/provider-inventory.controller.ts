...
export class ProviderInventoryController {
  ...
  @Get('lookup')
  @ApiOperation({ summary: 'Lookup active approved provider inventory items' })
  lookup(@CurrentUser() user: AuthUserContext) { return this.service.lookup(user); }

  @Post()
  @ApiOperation({ summary: 'Create provider inventory item with optional nested variants', description: 'PROVIDER only. providerId is derived from JWT; provider cannot approve/publish variants directly.' })
  ...
  @Patch(':id')
  @ApiOperation({ summary: 'Update own provider inventory item and upsert variants', description: 'Variant id must belong to the provider-owned gift. Material variant changes re-submit approved gifts for moderation; stock-only changes do not.' })
