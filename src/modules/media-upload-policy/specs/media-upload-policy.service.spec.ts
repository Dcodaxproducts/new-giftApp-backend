...
describe('Media upload policy source safety', () => {
  const service = readFileSync(join(__dirname, '../services/media-upload-policy.service.ts'), 'utf8');
  const repository = readFileSync(join(__dirname, '../repositories/media-upload-policy.repository.ts'), 'utf8');
  const controller = readFileSync(join(__dirname, '../controllers/media-upload-policy.controller.ts'), 'utf8');
