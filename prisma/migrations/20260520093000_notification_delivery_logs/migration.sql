...
CREATE UNIQUE INDEX IF NOT EXISTS "notification_delivery_logs_idempotency_key_key" ON "notification_delivery_logs"("idempotency_key");
CREATE INDEX IF NOT EXISTS "notification_delivery_logs_recipient_id_recipient_type_created_at_idx" ON "notification_delivery_logs"("recipient_id", "recipient_type", "created_at");
CREATE INDEX IF NOT EXISTS "notification_delivery_logs_notification_type_created_at_idx" ON "notification_delivery_logs"("notification_type", "created_at");
CREATE INDEX IF NOT EXISTS "notification_delivery_logs_in_app_status_socket_status_push_status_email_status_idx" ON "notification_delivery_logs"("in_app_status", "socket_status", "push_status", "email_status");

DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'notification_delivery_logs_notification_id_fkey') THEN
